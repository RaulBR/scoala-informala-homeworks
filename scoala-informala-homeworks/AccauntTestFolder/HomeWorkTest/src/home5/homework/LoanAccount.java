...
/**
 * This class is a chiled class of{@link AbstractAccaunt} and it models a lown
 * account.
 * 
 * @Raul
 */
public class LoanAccount extends AbstractAccaunt {
	 ...
	 * 
	 * @param loanAmount
	 *            the amount of money requested as a loan
	 * @param months
	 *            the amount of month till payment.
	 * 
	 */
	 ...
	 * 
	 * @param dayOfTHeMonth
	 *            the day the paiment is due (min the 20'th of the month)
	 * @param ammoutPerInstalment
	 *            the amount that needs to be payed.
	 * 
	 */
	public void payLoan(int dayOfTHeMonth, float ammoutPerInstalment)
			throws PayIsLessException, InsuficentMoneyException {
	...
	private float ammoutPerInstalment() {
		return (creditInterest() + loanAmount) / numberOfInstalents;
	}

	private void payDedLine(int dayOfTHeMonth, float ammoutPerInstalment)
			throws PayIsLessException, InsuficentMoneyException {
		if (dayOfTHeMonth > 20) {
		...
		if (dayOfTHeMonth > 20 && ammoutPerInstalment != ammoutPerInstalment() + 0.5f / 100 * ammoutPerInstalment()) {
